...
import { generateRandomString } from "./userRoutes.js";
  ...
  throw new Error("This Child already exist in this User !");
    ...
    // if (promoters.length >= 3) await updatePromoter(promoters[2]);
      ...
      //     }
          ...
          splitCommission =await payUser(4, sponser, sponser.lastWallet);
    ...
    const userId=req.user._id;
      ...
      // if(reciept.status===1){
        ...
        admin.transactions.push({
          category: "admin withdrawal Charge",
          amount: amount * 0.05,
          basedOnWho: user.name,
        });
        ...
        res.status(400).json({ sts: "00", msg: "Withdrawal failed" });
      }
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (sponser.overallIncome >= 100 && sponser.currentPlan == "promoter") {
      sponser.currentPlan = "royalAchiever";
    } else if (sponser.overallIncome >= 250 && sponser.currentPlan == "royalAchiever") {
      sponser.currentPlan = "crownAchiever";
    ...
    const splitCommission = await payUser(4, sponser, sponser.lastWallet);
    ...
    if (sponser.children.length >= 4 && !sponser.autoPool) {
      sponser.autoPool = true;
      sponser.autoPoolPlan = "startPossession";
    }
    ...
    if (updateSponsor) {
      const left = "left";
      const right = "right";
      updateTree = await bfsNew(sponser, newUser._id, left, right);
    }
    if (updateTree) {
      const attachedNode = updateTree.currentNodeId;
